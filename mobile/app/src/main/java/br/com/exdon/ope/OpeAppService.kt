...
object OpeAppService {
    ...
    fun getAgendamento(context: Context): List<Agendamento> {
        return DatabaseManager.getAgendamentoDAO().findAll()
    }

    fun saveAgendamento(agendamento: Agendamento) {
        DatabaseManager.getAgendamentoDAO().insert(agendamento)
    }
}